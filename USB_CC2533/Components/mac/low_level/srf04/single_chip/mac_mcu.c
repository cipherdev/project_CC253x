/**************************************************************************************************
  Filename:       mac_mcu.c
  Revised:        $Date: 2012-02-21 16:36:40 -0800 (Tue, 21 Feb 2012) $
  Revision:       $Revision: 29424 $
...
#ifdef TIMAC_RF4CE
    /* Create extra 128 bit random number */
    for (i = 0; i < sizeof(macStrongRandomSeed); i++)
    {
      uint8 j, seed = 0;
      for (j = 0; j < 8; j++)
      {
...
MAC_INTERNAL_API uint8 macMcuRandomByte(void)
{
  ...
  ADCCON1 = (ADCCON1 & (RCTRL_BITS ^ 0xFF)) | RCTRL_CLOCK_LFSR;
...
MAC_INTERNAL_API void macMcuOverflowSetCompare(uint32 count)
{
  ...
  {
    enableCompareInt = 1;
    T2IRQM &= (TIMER2_OVF_COMPARE1M ^ 0xFF);
  }
