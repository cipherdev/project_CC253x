...
void usbdpInit(void)
{
   usbdpData.pDesc = (const uint8 *)usbDescriptorMarker.pUsbDescStart;
} // usbdpInit
...
*/
void * usbdpFindNext(uint8 wantedType, uint8 haltAtType)
{
   void *pResult;
   pResult = NULL;

   // As long as we haven't reached the end...
   while (usbdpData.pDesc != (void *) usbDescriptorMarker.pUsbDescEnd) {

      // If we have a match on wantedType...
      if (usbdpData.pDesc[DESC_TYPE_IDX] == wantedType) {
         pResult = (void *) usbdpData.pDesc;
...
*/
USB_DEVICE_DESCRIPTOR * usbdpGetDeviceDesc(void)
{
   usbdpInit();
...
*     A pointer to the \ref USB_DEVICE_DESCRIPTOR, or \c NULL if it was not found.
*/
USB_CONFIGURATION_DESCRIPTOR * usbdpGetConfigurationDesc(uint8 cfgValue, uint8 cfgIndex)
{
   USB_CONFIGURATION_DESCRIPTOR *pConfigurationDesc;
   usbdpInit();
...
*/
USB_INTERFACE_DESCRIPTOR * usbdpGetInterfaceDesc(uint8 cfgValue, uint8 intNumber, uint8 altSetting)
{
...
{
   USB_STRING_DESCRIPTOR *pStringDesc;
   usbdpInit();
