...
static NO_INIT uint8 reqChannel;

static uint8 macPhyTxPower = MAC_RADIO_TX_POWER_DEFAULT;
static uint8 reqTxPower    = MAC_RADIO_TX_POWER_DEFAULT;
...
MAC_INTERNAL_API void macRadioUpdateTxPower(void)
{
  ...
  if (reqTxPower != macPhyTxPower)
  {
    ...
    if (!macRxOutgoingAckFlag && !MAC_TX_IS_PHYSICALLY_ACTIVE())
    {
      ...
      /* abstracted TX power register(s) control */
