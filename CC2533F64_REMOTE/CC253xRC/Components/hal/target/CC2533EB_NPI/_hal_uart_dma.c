/**************************************************************************************************
  ...
  A known defect is that when flow control is enabled, the function HalUARTPollTxTrigger() can
  prematurely invoke HAL_DMA_MAN_TRIGGER(HAL_DMA_CH_TX) and clobber that last byte of one txBuf[]
  block transfer with the first byte of the next txBuf[] block transfer.
