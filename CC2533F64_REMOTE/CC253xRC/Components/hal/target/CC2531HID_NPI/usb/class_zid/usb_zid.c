...
void usbHidProcessEvents(void)
{
    ...
    // Handle USB packets on OUT endpoints
    if ( USBIRQ_GET_EVENT_MASK() & USBIRQ_EVENT_OUT ) {
      // Handle USB packets on EP1 OUT
      if (USBIRQ_GET_EVENT_MASK() & USBIRQ_EVENT_EP1OUT) {
        USBIRQ_CLEAR_EVENTS(USBIRQ_EVENT_OUT);
    // Call application polling callback
        usbOutReportPoll(1);
      }
      // Handle USB packets on EP2 OUT
      if (USBIRQ_GET_EVENT_MASK() & USBIRQ_EVENT_EP2OUT) {
        USBIRQ_CLEAR_EVENTS(USBIRQ_EVENT_OUT);
        // Call application polling callback
        usbOutReportPoll(2);
      }
      // Handle USB packets on EP3 OUT
      if (USBIRQ_GET_EVENT_MASK() & USBIRQ_EVENT_EP3OUT) {
        USBIRQ_CLEAR_EVENTS(USBIRQ_EVENT_OUT);
        // Call application polling callback
        usbOutReportPoll(3);
      }
      // Handle USB packets on EP4 OUT
      if (USBIRQ_GET_EVENT_MASK() & USBIRQ_EVENT_EP4OUT) {
        USBIRQ_CLEAR_EVENTS(USBIRQ_EVENT_OUT);
        // Call application polling callback
        usbOutReportPoll(4);
      }
      // Handle USB packets on EP5 OUT
      if (USBIRQ_GET_EVENT_MASK() & USBIRQ_EVENT_EP5OUT) {
        USBIRQ_CLEAR_EVENTS(USBIRQ_EVENT_OUT);
        // Call application polling callback
        usbOutReportPoll(5);
      }
