...
void usbfwInit(void)
{
    // usbDescInit() must be called before usbfwInit().
    ...
    usbfwData.remoteWakeup = (usbdpGetConfigurationDesc(1, 0)->bmAttributes & 0x20) ? TRUE : FALSE;

    HAL_USB_ENABLE();
    HAL_USB_RUN();

    // What if we power up from a host which is in suspend. To trigger this
    // interrupt we enable it here. in addition the D+ line will not trigger
    // interrupt so we have to set the interrupt manually.
    USBCIE |= USBCIF_SUSPENDIF;
 ...
 */
void usbfwReadFifo(uint8 volatile *pFifo, uint8 count, void *pData)
{
   uint8 *pTemp = pData;
   if (count) {
      do {
 ...
 */
void usbfwWriteFifo(uint8 volatile *pFifo, uint8 count, void *pData)
{
   uint8 *pTemp = pData;
   if (count) {
      do {
