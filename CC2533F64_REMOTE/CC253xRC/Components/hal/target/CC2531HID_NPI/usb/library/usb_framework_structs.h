...
typedef struct {
    uint8 *pBuffer;  ///< Pointer to where IN/OUT data should be taken from/received
...
typedef struct {
    ...
    uint8 const *pDescStart;  ///< A pointer to the descriptor to be returned for the given index/value
    uint16 length;            ///< The length of the returned descriptor
} DESC_LUT_INFO;

/// Look-up table for double-buffer settings
typedef struct {
    USB_INTERFACE_DESCRIPTOR const *pInterface; ///< Pointer to an interface descriptor
...
typedef struct {
    uint8 const *pDesc; ///< Pointer to the current descriptor
} USBDP_DATA;
//@}
