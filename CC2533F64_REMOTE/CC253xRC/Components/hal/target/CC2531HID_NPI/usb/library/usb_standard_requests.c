...
void usbsrGetStatus(void)
{
   ...
   // Return status for device, interface, or endpoint
      ...
      if (usbfwData.ep0Status != EP_STALL) {
         // Send it
         usbSetupData.pBuffer = (uint8 *)&status;
...
void usbsrGetDescriptor(void)
{
   ...
   case DESC_TYPE_DEVICE:
      usbSetupData.pBuffer = (uint8 *) usbdpGetDeviceDesc();
      usbSetupData.bytesLeft = usbSetupData.pBuffer[DESC_LENGTH_IDX];
      break;

   // Configuration descriptor
   case DESC_TYPE_CONFIG:
      usbSetupData.pBuffer = (uint8 *) usbdpGetConfigurationDesc(0, LO_UINT16(usbSetupHeader.value));
   ...
   default:
      ...
      for (n = 0; n < ((uint16)usbDescriptorMarker.pUsbDescLutEnd - (uint16)usbDescriptorMarker.pUsbDescLut) / sizeof(DESC_LUT_INFO); n++) {
         ...
         {
            usbSetupData.pBuffer = (uint8 *)usbDescriptorMarker.pUsbDescLut[n].pDescStart;
 ...
 */
static void ConfigureEndpoints(USB_INTERFACE_DESCRIPTOR *pInterface)
{
   uint8 n;
   uint8 maxpRegValue;
   uint8 csRegValue;
   ...
   DBLBUF_LUT_INFO *pUsbDblbufLutInfo;

   // Locate the double buffer settings
   if (pInterface->bNumEndpoints) {
       pUsbDblbufLutInfo = (DBLBUF_LUT_INFO *) usbDescriptorMarker.pUsbDblbufLut;
...
{
   uint8 n;
   USB_CONFIGURATION_DESCRIPTOR *pConfiguration;
   ...
   // Default endpoint setup
      ...
      if (LO_UINT16(usbSetupHeader.value)) {
         ...
         if (pConfiguration) {
            ...
            // Must be called before using \ref usbdpFindNext()
            usbdpInit();
            // For each interface...
