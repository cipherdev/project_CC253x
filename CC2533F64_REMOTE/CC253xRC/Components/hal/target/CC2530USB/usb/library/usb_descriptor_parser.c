...
#endif
  {
    ...
    case 3:
      if (serialNumber[0] == 0)
      {
        ...
        HalFlashRead(HAL_FLASH_IEEE_PAGE, HAL_FLASH_IEEE_OSET, aExtendedAddress, 8);

        if (!memcmp(aExtendedAddress, nullAddr, 8))
        {
          // Attempt to read the extended address from the designated location in the Info Page.
          (void)memcpy(aExtendedAddress, (uint8 *)(P_INFOPAGE+HAL_INFOP_IEEE_OSET), 8);
        }
        ...
        for (uint8 idx = sizeof(serialNumber)-2, cnt = 0; cnt < 8; cnt++, idx -= 4)
        {
