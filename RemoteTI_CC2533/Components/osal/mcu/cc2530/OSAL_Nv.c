/**************************************************************************************************
  Filename:       OSAL_Nv.c
  Revised:        $Date: 2010-05-07 12:08:32 -0700 (Fri, 07 May 2010) $
  (void)Revision:       $Revision: 22413 $
...
#else
# error Please implement low Vdd check for this configuration.
#endif
    ...
    if ( (hdr.id != OSAL_NV_ZEROED_ID) && (hdr.id != skipId) )
    {
      if ( hdr.chk == calcChkF( srcPg, srcOff, hdr.len ) )
      {
        if ( writeItem( pgRes, hdr.id, hdr.len, NULL, FALSE ) )
        {
          ...
          dstOff -= OSAL_NV_HDR_SIZE;
          writeWordH( pgRes, (dstOff+OSAL_NV_HDR_CHK), (uint8 *)&chk );
          chk = hdr.chk;
          HalFlashRead(pgRes, dstOff, (uint8 *)(&hdr), OSAL_NV_HDR_SIZE);
...
uint8 osal_nv_item_init( uint16 id, uint16 len, void *buf )
{
  ...
  else if ( (findItem( id ) != OSAL_NV_ITEM_NULL) )
  {
    return SUCCESS;
  }
