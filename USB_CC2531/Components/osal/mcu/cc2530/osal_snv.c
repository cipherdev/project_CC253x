...
// If a rechargeable device is produced, this fix would be useful. Otherwise, a scenario in which
// the bus voltage drops below the minimum safe voltage for flash erasing and then rises back above
// it sustainably is not anticipated.
#if !defined OSAL_SNV_RECHARGEABLE
#define OSAL_SNV_RECHARGEABLE  FALSE
#endif
...
#if (!defined FEATURE_OAD && !defined FEATURE_SERIAL_BOOT && !defined FEATURE_SBL)
#pragma location="ZIGNV_ADDRESS_SPACE"
const __code __no_init uint8 _nvBuf[OSAL_NV_PAGES_USED * OSAL_NV_PAGE_SIZE];
...
static void writeWord( uint8 pg, uint16 offset, uint8 *pBuf )
{
  uint16 addr = (offset / HAL_FLASH_WORD_SIZE) +
 ...
 *          Otherwise, NV_OPER_FAILED for failure.
 */
uint8 osal_snv_makeRoomInActivePage(uint8 numOfItems, osalSnvLen_t lenTable[])
{
  uint16 alignedLen = 0;
  uint8 i;
  ...
  {
    alignedLen += ((lenTable[i] + OSAL_NV_WORD_SIZE - 1) / OSAL_NV_WORD_SIZE) * OSAL_NV_WORD_SIZE;
  }

  if ( pgOff + alignedLen + (OSAL_NV_WORD_SIZE * numOfItems) > OSAL_NV_PAGE_SIZE )
  {
    setXferPage();
    compactPage(activePg);
  }
