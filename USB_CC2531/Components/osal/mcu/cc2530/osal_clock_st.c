/**************************************************************************************************
  Filename:       osal_clock_st.c
  Revised:        $Date: 2011-04-11 10:53:58 -0700 (Mon, 11 Apr 2011) $
  Revision:       $Revision: 25649 $

  Description:    ST-only replacement for OSAL Clock definition and manipulation functions.
...
#define OSAL_CLOCK_ST_GET(STCNT) st (    \
  uint32 T32 = 0;                \
                                 \
  do {                           \
    (STCNT) = T32;               \
...
void osalTimeUpdate(void)
{
  ...
  {
    run = 1;
    OSAL_CLOCK_ST_GET(stLast);
    return;
  }

  uint32 stCurr;
  OSAL_CLOCK_ST_GET(stCurr);  // Get the free-running count of 30.5 usec timer ticks.
 ...
 * @fn          osal_getClock
 *
 * @brief       This function is a stub for the call in rcn_seckey.c for random number seed.
 *
 * input parameters
 *
 * @param       None.
 ...
 * None.
 *
 * @return      A number that should be equally random to the value returned by the OSAL function.
 **************************************************************************************************
 */
