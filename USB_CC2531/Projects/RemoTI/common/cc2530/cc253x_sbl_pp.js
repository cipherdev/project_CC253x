...
// void main(void)
{
  ...
  case "ProdSBL":
    ...
    WshShell.Run("cc253x_sim2bin.exe tmp.sim tmp.bin", 8, true);
  ...
  case "ProdHex":
    var sblFile = WScript.Arguments(2);
    var rootCnt = WScript.Arguments(3);
    var fsbl = fso.OpenTextFile(sblFile, ForReading);
    fout = fso.CreateTextFile(ppPath + ".hex", true);

    // SBL is split between root (intercepting IVEC's) and the last or lock-bits page.
    while (rootCnt != 0)
    {
      var s = fsbl.ReadLine();
      fout.WriteLine(s);
    ...
    // Look to throw away the header line to the banked area of the SBL image.
    var s = fsbl.ReadLine();
    if (s.substr(1,1) != "0")
    {
      fout.WriteLine(s);
